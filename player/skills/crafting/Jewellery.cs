...
namespace WorldServer.player.skills.crafting
{
    class Jewellery : CraftingData
    {
        public Jewellery() {
	    }
	    ...
	    private static void displayJewellery(Player p, int index) {
		    ...
		    for (int i = 1; i < items.Length; i++) { // i is set to 1 to ignore the gold
			    for (int j = 0; j < items[i].Length; j++) {
				    if (p.getInventory().hasItem((int) GEMS[i - 1][1]) && p.getSkills().getGreaterLevel(Skills.SKILL.CRAFTING) >= (int)items[i][1]) { // lower it down 1 because of the gold..0 in jeweller is 0, but in GEMS is a sapphire
