...
using WorldServer.minigames.fightcave;
using WorldServer.player.skills.runecrafting;
using WorldServer.player.skills.farming;
...
namespace WorldServer.player
{
    class Player : Entity
    {
	    ...
	    public void decreasePrayerPoints(double modification) {
		    int lvlBefore = (int) Math.Ceiling(prayerPoints);
		    if(prayerPoints > 0) {
			    prayerPoints = (prayerPoints - modification <= 0 ? 0 : prayerPoints - modification);
		    }
		    int lvlAfter = (int) Math.Ceiling(prayerPoints);
