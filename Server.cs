...
using WorldServer.clans;
using WorldServer.minigames;
using WorldServer.grandexchange;
...
namespace WorldServer
{
    class Server
    {
        ...
        static readonly object eventsLocker = new object();
        ...
        public static void Main(string[] args)
        {
            ...
            Console.WriteLine("For future updates visit http://razorblade.servegame.com forums");
            ...
            Console.Title = "WorldServer Release";
            // setup the listener
            try
            {
                ...
                Console.WriteLine("World server started on port " + Constants.SERVER_PORT);
            }
            ...
            while (isRunning) {
                ...
                {
                    Console.Title = "World server [Players: " + players.Count + "][Connections: " + connections.Count + "]";
                    lastInfoTime = curTime;
                    toggledStats = true;
                } else if (curTime - lastInfoTime >= 4000 && toggledStats) {
                    Console.Title = "World server [Events Running: " + events.Count + "]";
                    lastInfoTime = curTime;
                    toggledStats = false;
                }
                ...
                lock (players)
                {
                    ...
                    {
                        if (p == null)
                            misc.WriteError("Didn't think this was possible but this is why server crashed?");
                        if (p.isActive()) {
                ...
                try { System.Threading.Thread.Sleep(500 - (Environment.TickCount-curTime) ); }catch { }
            }
